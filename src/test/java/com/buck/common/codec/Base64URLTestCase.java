 ...
 */
public class Base64URLTestCase {
    private void runTestVector(byte[] expected, byte[] inEncoded) {
        Codec codec = Codec.forName("base64urlsafe");
    ...
    private void runSymmetricTestVector(byte[] bytes) {
        Codec codec = Codec.forName("base64url");
    ...
    public void testExistence() {
        {
            Assert.assertTrue(Codec.isSupported("Base64URL"));
        }
        {
            Set<String> aliases = Codec.forName("Base64URL").aliases();
            Assert.assertTrue(aliases.contains("base64URLSafe"));
        }
    }
